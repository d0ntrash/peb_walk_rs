...
// fn dyn_resolve_with_api(dll_name: &str, function_name: &str) -> FARPROC {
//     unsafe{
//         let handle: HINSTANCE = LoadLibraryA(dll_name.as_ptr());
//         let fpMessageBoxA: FARPROC = GetProcAddress(
//            handle, 
//             function_name.as_ptr()
//         );
//         let ptr = fpMessageBoxA.unwrap() as usize;
//         println!("prt: {ptr:x}");
//         fpMessageBoxA
//     }
// }
...
fn get_module_base_addr(module_name: &str) -> HINSTANCE {
    unsafe {
        let peb_offset: *const u64 = utils::__readgsqword(0x60)  as *const u64;
        let rf_peb: *const PEB = peb_offset as * const PEB;
        let peb = *rf_peb;

        // let debug = peb.BeingDebugged;
